...
public class Building implements Buildingc, Entityc, Healthc, IndexableEntity__all, IndexableEntity__build, Posc, Teamc, Timerc {
  ...
  public boolean allowUpdate() {
        return team != Team.derelict && block.supportsEnv(state.rules.env) && (tile instanceof EditorTile || block.privileged || !state.rules.limitMapArea || !state.rules.disableOutsideArea || Rect.contains(state.rules.limitX, state.rules.limitY, state.rules.limitWidth, state.rules.limitHeight, tile.x, tile.y));
  }
  ...
  public boolean checkSuppression() {
        if (isHealSuppressed()) {
            if (Mathf.chanceDelta(0.03)) {
                Fx.regenSuppressParticle.at(x + Mathf.range(block.size * tilesize / 2.0F - 1.0F), y + Mathf.range(block.size * tilesize / 2.0F - 1.0F), suppressColor);
            }
            return true;
        }
        return false;
  ...
  public int moduleBitmask() {
        return (items != null ? 1 : 0) | (power != null ? 1 << 1 : 0) | (liquids != null ? 1 << 2 : 0) | 1 << 3 | (timeScale != 1.0F ? 1 << 4 : 0) | (lastDisabler != null && lastDisabler.isValid() ? 1 << 5 : 0);
  }
  ...
  public void drawItemSelection(UnlockableContent selection) {
        if (selection != null) {
            float dx = x - block.size * tilesize / 2.0F;
            float dy = y + block.size * tilesize / 2.0F;
            float s = iconSmall / 4.0F * selection.fullIcon.ratio();
            float h = iconSmall / 4.0F;
  ...
  public void onDeconstructed(Unit builder) {
        if (liquids != null && liquids.currentAmount() > 0 && (!liquids.current().incinerable || block.deconstructDropAllLiquid)) {
            float perCell = liquids.currentAmount() / (block.size * block.size) * 2.0F;
  ...
  public void onDestroyed() {
        ...
        Damage.dynamicExplosion(x, y, flammability * block.flammabilityScale, explosiveness * 3.5F * block.explosivenessScale, power, tilesize * block.size / 2.0F, state.rules.damageExplosions, block.destroyEffect, block.baseShake);
